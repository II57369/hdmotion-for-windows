...
int ButterflySeek(DWORD diskNumber, long long times, int Resolution)  // 磁盘号 + 读取次数 + 分辨率（每个周期需要读取几次） 
{
//	printf("reading %d...\n", lbaAddress);
	long long cnt=0;
    HANDLE hDevice;
    ...
    random_device rd;   // non-deterministic generator
    mt19937_64 gen(rd());  // to seed mersenne twister.
    uniform_int_distribution<long long> dist(0, MaxLBA/Resolution/2); // distribute results between 1 and 6 inclusive.
...
int main(){
    ...
    ButterflySeek(drvnum, 10000000, 100);
	return 0;
}
